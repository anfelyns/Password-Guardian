from PyQt5.QtWidgets import (
    ...
    QFrame, QProgressBar, QMessageBox, QCheckBox, QApplication, QSizePolicy
)
...
class LoginModal(QDialog):
    ...
    def init_ui(self):
        ...
        """)
        layout = QVBoxLayout(self); layout.setContentsMargins(40,40,40,40); layout.setSpacing(22)

        head = QVBoxLayout(); head.setSpacing(12); head.setAlignment(Qt.AlignCenter)
        icon = QLabel("🔐"); icon.setStyleSheet("font-size:46px;")
        title = QLabel("Connexion"); title.setFont(QFont("Segoe UI", 24, QFont.Bold))
        title.setStyleSheet(Styles.get_label_style(24))
        sub = QLabel("Accédez à votre coffre-fort sécurisé"); sub.setAlignment(Qt.AlignCenter)
        sub.setStyleSheet(Styles.get_label_style(14, Styles.TEXT_SECONDARY))
        head.addWidget(icon); head.addWidget(title); head.addWidget(sub)
        layout.addLayout(head)

        form = QVBoxLayout(); form.setSpacing(18)

        lbl = QLabel("📧 Adresse e-mail")
        lbl.setStyleSheet(f"color: {Styles.TEXT_SECONDARY}; font-size: 13px; font-weight: normal;")
        self.email_input = QLineEdit(); style_line_edit(self.email_input)
        ...
        row = QHBoxLayout()
        self.password_input = QLineEdit(); self.password_input.setEchoMode(QLineEdit.Password); style_line_edit(self.password_input)
        self.password_input.setPlaceholderText("Entrez votre mot de passe")
        self.toggle_pwd_btn = QPushButton("👁️"); self.toggle_pwd_btn.setFixedSize(40,40); self.toggle_pwd_btn.setCheckable(True)
        ...
        """)
        self.toggle_pwd_btn.toggled.connect(lambda c: self.password_input.setEchoMode(QLineEdit.Normal if c else QLineEdit.Password))
        row.addWidget(self.password_input); row.addWidget(self.toggle_pwd_btn)
        form.addLayout(row)
        ...
        footer = QHBoxLayout(); footer.setAlignment(Qt.AlignCenter)
        t = QLabel("Nouveau chez SecureVault?"); t.setStyleSheet(Styles.get_label_style(14, Styles.TEXT_MUTED))
        link = QLabel("<a href='#' style='color:#60a5fa; text-decoration:none; font-weight:bold;'>Créer un compte</a>")
        link.setOpenExternalLinks(False); link.linkActivated.connect(lambda: self.switch_to_register.emit())
        footer.addWidget(t); footer.addWidget(link)
        layout.addLayout(footer)

    def on_login(self):
        email = self.email_input.text().strip()
        pwd = self.password_input.text()
        if not email: QMessageBox.warning(self, "Erreur", "Veuillez saisir votre adresse email"); return
        if not pwd: QMessageBox.warning(self, "Erreur", "Veuillez saisir votre mot de passe"); return
        if '@' not in email or '.' not in email: QMessageBox.warning(self, "Erreur", "Adresse email invalide"); return
        self.login_success.emit(email, pwd); self.accept()
...
class RegisterModal(QDialog):
    ...
    def init_ui(self):
        ...
        head = QVBoxLayout(); head.setAlignment(Qt.AlignCenter); head.setSpacing(10)
        icon = QLabel("🛡️"); icon.setStyleSheet("font-size:46px;")
        title = QLabel("Créer un compte"); title.setFont(QFont("Segoe UI", 24, QFont.Bold))
        title.setStyleSheet(Styles.get_label_style(24))
        sub = QLabel("Rejoignez la communauté SecureVault"); sub.setAlignment(Qt.AlignCenter)
        sub.setStyleSheet(Styles.get_label_style(14, Styles.TEXT_SECONDARY))
        head.addWidget(icon); head.addWidget(title); head.addWidget(sub)
        layout.addLayout(head)
        ...
        self.name_input = QLineEdit(); style_line_edit(self.name_input)
        ...
        prow.addWidget(self.password_input); prow.addWidget(self.toggle_pwd_btn)
        form.addLayout(prow)
        ...
        self.weak_password_container.setStyleSheet(f"""
            QFrame {{
        ...
        self.confirm_input = QLineEdit(); self.confirm_input.setEchoMode(QLineEdit.Password); style_line_edit(self.confirm_input)
        ...
        btn.setStyleSheet(S.get_button_style(primary=True))
        btn.setMinimumHeight(50)
        btn.clicked.connect(self.on_register)
        layout.addWidget(btn)

        foot = QHBoxLayout(); foot.setAlignment(Qt.AlignCenter)
        t = QLabel("Déjà un compte?"); t.setStyleSheet(S.get_label_style(14, S.TEXT_MUTED))
        link = QLabel("<a href='#' style='color:#60a5fa; text-decoration:none; font-weight:bold;'>Se connecter</a>")
        link.setOpenExternalLinks(False); link.linkActivated.connect(lambda: self.switch_to_login.emit())
        foot.addWidget(t); foot.addWidget(link)
    ...
    def on_register(self):
        ...
        if not name: QMessageBox.warning(self, "Erreur", "Veuillez saisir votre nom complet"); return
        if not email: QMessageBox.warning(self, "Erreur", "Veuillez saisir votre adresse e-mail"); return
        if '@' not in email or '.' not in email: QMessageBox.warning(self, "Erreur", "Adresse e-mail invalide"); return
        if not pwd: QMessageBox.warning(self, "Erreur", "Veuillez créer un mot de passe maître"); return
        if pwd != cpwd: QMessageBox.warning(self, "Erreur", "Les mots de passe ne correspondent pas"); return
        ...
        if strength == "weak":
            ...
            if reply == QMessageBox.Yes:
                ...
                return  # Let user review the generated password
        
        self.register_success.emit(name, email, pwd); self.accept()
...
class AddPasswordModal(QDialog):
    ...
    def init_ui(self):
        ...
        """)
        layout = QVBoxLayout(self); layout.setContentsMargins(40,40,40,40); layout.setSpacing(16)

        head = QHBoxLayout()
        icon = QLabel("🔐"); icon.setStyleSheet("font-size:30px;")
        from gui.styles.styles import Styles as S
        title = QLabel("Nouveau Mot de Passe"); title.setFont(QFont("Segoe UI", 22, QFont.Bold))
        title.setStyleSheet(S.get_label_style(22))
        head.addWidget(icon); head.addWidget(title); head.addStretch()
        layout.addLayout(head)

        form = QVBoxLayout(); form.setSpacing(12)

        lab = QLabel("🌐 Nom du site"); lab.setStyleSheet(S.get_label_style(14, S.TEXT_SECONDARY))
        self.site_input = QLineEdit(); style_line_edit(self.site_input)
        self.site_input.setPlaceholderText("ex: google.com, github.com...")
        form.addWidget(lab); form.addWidget(self.site_input)

        lab2 = QLabel("📧 Email / Identifiant"); lab2.setStyleSheet(S.get_label_style(14, S.TEXT_SECONDARY))
        self.email_input = QLineEdit(); style_line_edit(self.email_input)
        self.email_input.setPlaceholderText("votre@email.com ou nom d'utilisateur")
        form.addWidget(lab2); form.addWidget(self.email_input)

        lab3 = QLabel("🔒 Mot de passe"); lab3.setStyleSheet(S.get_label_style(14, S.TEXT_SECONDARY))
        row = QHBoxLayout()
        self.pwd_input = QLineEdit(); self.pwd_input.setEchoMode(QLineEdit.Password); style_line_edit(self.pwd_input)
        ...
        self.toggle_pwd_btn.setStyleSheet("""
            ...
            QPushButton:checked { background-color: rgba(59,130,246,0.3); }
        """)
        self.toggle_pwd_btn.toggled.connect(lambda c: self.pwd_input.setEchoMode(QLineEdit.Normal if c else QLineEdit.Password))
        row.addWidget(self.pwd_input); row.addWidget(self.toggle_pwd_btn)
        form.addWidget(lab3); form.addLayout(row)

        gen = QHBoxLayout()
        self.generate_checkbox = QCheckBox("🎲 Générer un mot de passe sécurisé")
        self.generate_checkbox.setStyleSheet(S.get_label_style(14, S.TEXT_PRIMARY))
        ...
        self.generate_btn.setStyleSheet(S.get_button_style(primary=False))
        gen.addWidget(self.generate_btn); gen.addStretch()
        form.addLayout(gen)
        ...
        cl = QLabel("📁 Catégorie"); cl.setStyleSheet(S.get_label_style(14, S.TEXT_SECONDARY))
        self.category_combo = QComboBox()
        self.category_combo.addItems(["👤 Personnel", "💼 Travail", "💳 Finance", "🎮 Jeux", "📚 Étude", "📁 Autre"])
        self.category_combo.setMinimumHeight(44); self.category_combo.setFont(QFont("Segoe UI", 12))
        self.category_combo.setStyleSheet(S.get_input_style())
        form.addWidget(cl); form.addWidget(self.category_combo)

        layout.addLayout(form)

        btns = QHBoxLayout(); btns.setSpacing(12)
        cancel = AnimatedButton("❌ Annuler"); cancel.setStyleSheet(S.get_button_style(primary=False)); cancel.setMinimumHeight(48)
        save = AnimatedButton("💾 Enregistrer le mot de passe"); save.setStyleSheet(S.get_button_style(primary=True)); save.setMinimumHeight(48)
        cancel.clicked.connect(self.reject); save.clicked.connect(self.on_save)
    ...
    def toggle_password_generation(self, checked):
        ...
        if checked:
            ...
            self.pwd_input.style().unpolish(self.pwd_input); self.pwd_input.style().polish(self.pwd_input)
    ...
    def generate_password(self):
        ...
        self.pwd_input.setText(pwd); self.strength_widget.update_strength(pwd)

    def on_save(self):
        site = self.site_input.text().strip()
        user = self.email_input.text().strip()
        pwd = self.pwd_input.text().strip()
        if not site: QMessageBox.warning(self, "Erreur", "Veuillez saisir un nom de site"); return
        if not user: QMessageBox.warning(self, "Erreur", "Veuillez saisir un email/identifiant"); return
        if not pwd: QMessageBox.warning(self, "Erreur", "Veuillez saisir un mot de passe"); return
        cat = self.category_combo.currentText().replace("👤 ","").replace("💼 ","").replace("💳 ","").replace("📁 ","").lower()
        self.password_added.emit({'site_name': site, 'username': user, 'password': pwd, 'category': cat})
        self.accept()
...
class ViewPasswordModal(QDialog):
    ...
    def init_ui(self):
        ...
        """)
        layout = QVBoxLayout(self); layout.setContentsMargins(30,30,30,30); layout.setSpacing(16)

        head = QHBoxLayout()
        icon = QLabel(self.password_data.get('site_icon', '🔒')); icon.setStyleSheet("font-size:22px;")
        title = QLabel(self.password_data['site_name']); title.setFont(QFont("Segoe UI",18,QFont.Bold))
        from gui.styles.styles import Styles as S
        title.setStyleSheet(S.get_label_style(18))
        head.addWidget(icon); head.addWidget(title); head.addStretch()
        layout.addLayout(head)

        info = QVBoxLayout(); info.setSpacing(12)

        l1 = QVBoxLayout()
        lbl_id = QLabel("📧 Identifiant"); lbl_id.setStyleSheet(S.get_label_style(12, S.TEXT_MUTED))
        val_id = QLabel(self.password_data['username']); val_id.setStyleSheet(S.get_label_style(14, S.TEXT_SECONDARY))
        l1.addWidget(lbl_id); l1.addWidget(val_id); info.addLayout(l1)

        l2 = QVBoxLayout()
        lbl_pwd = QLabel("🔒 Mot de passe"); lbl_pwd.setStyleSheet(S.get_label_style(12, S.TEXT_MUTED))
        row = QHBoxLayout()
        ...
        """)
        copy = AnimatedButton("📋"); copy.setFixedSize(40,40); copy.setStyleSheet(S.get_button_style(False))
        copy.clicked.connect(self.copy_password)
        row.addWidget(val_pwd, 1); row.addWidget(copy)
        l2.addWidget(lbl_pwd); l2.addLayout(row); info.addLayout(l2)

        layout.addLayout(info); layout.addStretch()
        close = AnimatedButton("Fermer"); close.setStyleSheet(S.get_button_style(True)); close.setMinimumHeight(44); close.clicked.connect(self.accept)
...
class EditPasswordModal(QDialog):
    password_updated = pyqtSignal(int, str, str)  # (id, new_password, last_modified_str)
    ...
    def init_ui(self):
        ...
        lbl_old = QLabel("Ancien mot de passe"); lbl_old.setStyleSheet(Styles.get_label_style(13, Styles.TEXT_SECONDARY))
        self.in_old = QLineEdit(); self.in_old.setEchoMode(QLineEdit.Password); style_line_edit(self.in_old)
        layout.addWidget(lbl_old); layout.addWidget(self.in_old)

        lbl_new = QLabel("Nouveau mot de passe"); lbl_new.setStyleSheet(Styles.get_label_style(13, Styles.TEXT_SECONDARY))
        self.in_new = QLineEdit(); self.in_new.setEchoMode(QLineEdit.Password); style_line_edit(self.in_new)
        layout.addWidget(lbl_new); layout.addWidget(self.in_new)

        lbl_rep = QLabel("Répéter le nouveau mot de passe"); lbl_rep.setStyleSheet(Styles.get_label_style(13, Styles.TEXT_SECONDARY))
        self.in_rep = QLineEdit(); self.in_rep.setEchoMode(QLineEdit.Password); style_line_edit(self.in_rep)
        layout.addWidget(lbl_rep); layout.addWidget(self.in_rep)
        ...
        cancel = AnimatedButton("Annuler"); cancel.setMinimumHeight(48); cancel.setStyleSheet(Styles.get_button_style(False))
        save = AnimatedButton("💾 Mettre à jour"); save.setMinimumHeight(48); save.setStyleSheet(Styles.get_button_style(True))
        cancel.clicked.connect(self.reject); save.clicked.connect(self.on_save)
    ...
    def on_save(self):
        ...
        if not old or not new or not rep:
            QMessageBox.warning(self, "Champs manquants", "Veuillez remplir tous les champs."); return
        current = self.password_data.get('encrypted_password', '') or self.password_data.get('password', '')
        if old != current:
            QMessageBox.warning(self, "Ancien mot de passe", "L'ancien mot de passe est incorrect."); return
        if new != rep:
            QMessageBox.warning(self, "Confirmation", "La confirmation ne correspond pas."); return
...
class TwoFactorModal(QDialog):
    ...
    def verify_code(self):
        # This will be called by the main window
        # The verification logic is handled in main_window.py
        pass